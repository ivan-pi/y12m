...
c   called immediately after the call of   y12mcf.   the parameters
...
c   a   computed by   y12mcf   is not destroyed). subroutine  y12mhf
c   should be called before the call of subroutine   y12mcf (this
...
c
      ...
      integer   snr, ha
      double precision  a, w,    pivot, rcond, anorm
...
c
c
      double precision  aa, ynorm, znorm, t
...
c
   ...
   20 w(i)=0.0d0
      ...
      l=snr(j)
   30 w(l)=w(l)+t*a(j)
   40 if(w(i).gt.0.0d0) w(i)=w(i)+1.0d0
      if(w(i).le.0.0d0) w(i)=w(i)-1.0d0
   50 w(i)=-w(i)/pivot(i)
c
c
